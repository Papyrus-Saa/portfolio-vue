...
<template>
  <section class="education-section p-2 mx-auto rounded-md mb-10">
    <h2 class="text-xl font-semibold mb-4text-center">Bildung und Weiterbildung</h2>
    <ul class="space-y-3 p-6">
      ...
      >
        {{ item.title }} <OpenSetColor class="hover:bg-gray-500 cursor-pointer duration-500 rounded-sm" @click.stop="selectItem(index)"/>
      </li>
    </ul>

    <div
      v-if="selectedIndex !== null"
      class="education-card p-4"
    >
...
.education-card {
  ...
  box-shadow: inset 0px 0px 29px 0px rgb(122, 122, 122);
}
